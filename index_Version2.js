...
const PINO_LEVEL = process.env.PINO_LEVEL || "silent"; // set to 'debug' for more details
...
async function createPairingSession(phone, chatId) {
  ...
  // Wait for socket to become 'open' OR timeout and continue
  await new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      if (!resolved) {
        resolved = true;
        console.warn("Timed out waiting for connection 'open' — proceeding to request pairing code (socket may still be connecting).");
