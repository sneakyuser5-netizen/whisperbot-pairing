...
async function createPairingSession(phone, chatId) {
  ...
  // We'll capture the latest QR if any arrives
  let latestQR = null;
  ...
  // Wait up to TIMEOUT_MS for either 'open' or a QR update.
  const waitForOpenOrQR = () =>
    new Promise((resolve) => {
      let resolved = false;
  ...
  const result = await waitForOpenOrQR();

  if (result.type === "timeout") {
    console.warn("Socket did not reach 'open' or emit a QR within 60s — aborting pairing request. Please try /pair again.");
  ...
  // If we got an 'open', just proceed to request pairing code (socket fully connected).
  // If we got a 'qr', proceed as well (we will use the qr that was emitted).
  ...
  // Request a single pairing code (socket may be open or we have a QR)
  try {
    const code = await sock.requestPairingCode(phone);
    console.log(`PAIRING CODE for ${phone}: ${code}`);

    // send pairing code plus QR link if available
    let msg =
    ...
    if (latestQR) {
      msg += `If possible, scan the QR instead (faster). QR link:\n${latestQR}\n\n`;
    }
